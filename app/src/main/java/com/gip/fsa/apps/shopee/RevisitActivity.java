...
import com.gip.fsa.apps.shopee.adapter.RevisitShopeeAdapter;
...
public class RevisitActivity extends AppCompatActivity {
    ...
    private RevisitShopeeAdapter shopeeAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.shopee_revisit);
        _initialize();
    }
    ...
    void _initialize() {
        toolbar             = findViewById(R.id.shopee_revisit_toolbar);
        recyclerView        = findViewById(R.id.shopee_revisit_recycle);
        swipeRefreshLayout  = findViewById(R.id.shopee_revisit_swipe);
        constantUtility     = new ConstantUtility();
        ...
        toolbar.setTitle("Revisit (Shopee)");
    ...
    void _gridList() {
        progressDialog = new ProgressDialog(RevisitActivity.this);
        ...
        final Call<ShopeeModel> _model = _interface._goRevisit(_accountId, "0", _datetime, _signature);
        ...
        _model.enqueue(new Callback<ShopeeModel>() {
            ...
            public void onResponse(Call<ShopeeModel> call, final Response<ShopeeModel> response) {
                ...
                if(response.body() != null) {
                    if(response.body().getSuccess().matches("false")) {
                        Toast.makeText(RevisitActivity.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                    } else {
                        shopeeModel   = response.body();
                        shopeeAdapter = new RevisitShopeeAdapter(shopeeModel);
                        recyclerView.setAdapter(shopeeAdapter);
                ...
                {
                    Toast.makeText(RevisitActivity.this, ConstantUtility.ERROR_EXCEPTION, Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(Call<ShopeeModel> call, Throwable throwable) {
                progressDialog.dismiss();
                Toast.makeText(RevisitActivity.this, constantUtility.ERROR_API, Toast.LENGTH_SHORT).show();
            }
        });
    }
    void _gridNoLoading() {
    ...
    void _more() {
        ...
        shopeeAdapter.notifyItemInserted(shopeeModel.getDatas().size() - 1);
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                shopeeAdapter.notifyItemRemoved(modelSize);
                ...
                ShopeeInterface _interface     = RetrofitService.getRetrofitService().create(ShopeeInterface.class);
                final Call<ShopeeModel> _model = _interface._goRevisit(_accountId, String.valueOf(modelSize), _datetime, _signature);
                ...
                _model.enqueue(new Callback<ShopeeModel>() {
                    ...
                    public void onResponse(Call<ShopeeModel> call, final Response<ShopeeModel> response) {
                        if(response.body() != null) {
                            ...
                            shopeeAdapter.notifyDataSetChanged();
                            isLoading = false;
                        }
                        else
                        {
